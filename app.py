...
def safe_numeric_conversion(df, columns):
    """Safely convert columns to numeric types, avoiding BigInt serialization issues"""
    for col in columns:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
            # Convert to standard Python int/float to avoid BigInt issues
...
if not combined_df.empty:
    ...
    if not selected_brokers:
        ...
        if filtered_df.empty:
            ...
            # Safe numeric conversion to avoid BigInt issues
            main_table_df = safe_numeric_conversion(main_table_df, ["Value", "Percentage"])
...
if not combined_df.empty:
    ...
    def generate_ranking_table(df: pd.DataFrame, column: str):
        ...
        ranked_df = safe_numeric_conversion(ranked_df, [column, "Market Share"])
        ranked_df["Peringkat"] = ranked_df["Peringkat"].astype(int)
